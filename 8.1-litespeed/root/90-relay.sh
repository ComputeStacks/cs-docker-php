...
RELAY_PHP=$(/usr/local/lsws/lsphp81/bin/php-config --version | cut -c -3)  # 8.1
RELAY_INI_DIR=$(/usr/local/lsws/lsphp81/bin/php-config --ini-dir)          # /etc/php/8.1/cli/conf.d/
RELAY_EXT_DIR=$(/usr/local/lsws/lsphp81/bin/php-config --extension-dir)    # /usr/lib/php/20210902
...
# if RELAY_MEMORY is not defined, set to default value of 128m
if [ -z "$RELAY_MEMORY" ]; then
    RELAY_MEMORY=128M
fi
...
if $RELAY_ENABLED != "true"; then
    sed -i "s/^;\? \?relay.maxmemory =.*/relay.maxmemory = 0/" $RELAY_INI
fi
