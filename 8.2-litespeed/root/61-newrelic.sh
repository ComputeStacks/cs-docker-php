...
NR_PHP=$(/usr/local/lsws/lsphp82/bin/php-config --extension-dir | awk -F/ '{print $NF}') 
NR_INI_DIR=$(/usr/local/lsws/lsphp82/bin/php-config --ini-dir)
NR_EXT_DIR=$(/usr/local/lsws/lsphp82/bin/php-config --extension-dir)
...
if [[ -z "${NEW_RELIC_KEY}" ]]; then
  ...
  if [ -f $NR_EXT_DIR/newrelic.so ]; then
    rm $NR_EXT_DIR/newrelic.so
  fi

  cp /usr/src/newrelic/agent/x64/newrelic-$NR_PHP.so $NR_EXT_DIR/newrelic.so
