...
if [ -z ${MONARX_ID} ] || [ -z ${MONARX_SECRET} ]; then
  ...
  # grab latest file
  if [ -f /usr/local/lsws/lsphp82/lib/php/20220829/monarxprotect-php82.so ]; then
    rm /usr/local/lsws/lsphp82/lib/php/20220829/monarxprotect-php82.so
  fi
  cp /usr/lib/monarx-protect/monarxprotect-php82.so /usr/local/lsws/lsphp82/lib/php/20220829/
  echo "extension=monarxprotect-php82.so" > /usr/local/lsws/lsphp82/etc/php/8.2/mods-available/monarxprotect.ini
